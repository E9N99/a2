...
from .core.session import blblub
...
try:
    ...
    blblub.loop.run_until_complete(autovars())
...
if not Config.ALIVE_NAME:
    try:
        ...
        blblub.loop.run_until_complete(autoname())
...
try:
    ...
    blblub.loop.run_until_complete(setup_bot())
...
try:
    ...
    blblub.loop.run_until_complete(mybot())
...
try:
    LOGS.info("⌭ جـارِ تفعيـل الاشتـراك ⌭")
    blblub.loop.create_task(supscrips())
    LOGS.info("✓ تـم تفعيـل الاشتـراك .. بنجـاح ✓")
except Exception as e:
    LOGS.error(f"- {e}")
...
blblub.loop.run_until_complete(startup_process())
